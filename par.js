	...
	app.controller("parallelctrl", function($scope,$http,$rootScope, $filter) {
		...
		$scope.gridOptions = { 
		        data: 'test',
		        /*columnDefs: [{field: 'city_name', displayName: 'city_name'}, 
		                     {field:'Login_Time', displayName:'Login_Time'},
		                     {field:'Logout_Time', displayName:'Logout Time'},
		                     {field:'Online_Time_in_Mins', displayName:'Online Time in Mins'}
		                     ],*/
		...
		$scope.hoverIn = function(rowItem) {
			...
			//alert(rowItem.entity);
			console.log("entity ",rowItem.entity);
	    ...
	    $scope.hoverOut = function() {
		 ...
		 /*$scope.$on('ngGridEventData', function(){
		       highlight( $scope.gridOptions.selectRow(0, true));
		    });*/
	...
	console.log(h + m[0] + m[2]);
	console.log(h+","+ m[0]+"," + m[2]+","+ m[3]);
	 ...
	 console.log("Length : "+n_dimensions+" dimensions : "+dimensions);
	  ...
	  g.append("svg:g")
	    ...
	    .append("svg:text")
	      ...
	      .attr("y", function(d,i) { return i%2 == 0 ? la_bot : la_bot1 })
	      .attr("x", 0)
	      .attr("class", "label")
	      .text(String)
	      .append("title")
		  ...
		  for(key in d){
			  ...
			  /*if(hasWhiteSpace(key))
	  			{	
			  		...
			  		o[key.replace( /\s/g, '_')]=d[key];
	  			}
			  else*/
				  o[key]=d[key];
			  break;
		  }
		  //console.log(o.hasOwnProperty("city_name"));
		  //console.log("exp : "+o.city_name);
		  for(k in o){
	...
	$scope.data_table = function(sample) { 
		...
		for (var key=0;key<$scope.sample.length;key++)
			...
			if(key!="contains")
				{
			//console.log("k : ", key ," val : ",$scope.sample[key]);
			var tmp=[];
			for (j in $scope.sample[key])
				{
				//console.log("j : ", j ," val : ",$scope.sample[key][j]);
				if(hasWhiteSpace(j))
		  			{	
	...
	/*    $scope.tableParams = new ngTableParams({
	        ...
	        total: $scope.test.length,
	        getData: function($defer, params) {
	            var orderedData = params.sorting() ?
	                    $filter('orderBy')($scope.test, $scope.tableParams.orderBy()) :
	                    	$scope.test;
	...
	function highlight(d) {
	 ...
	 /* d3.select(this).attr("title",function(d,i)
	        ...
	        for (var key in d[i])
	        {
	          info += "<tr><td><b><i>" + key + "</i></b></td><td>" + d[i][key] + "</td></tr>"
	        }
	        return info + "</table>";
	      });*/
	}
	...
	//console.log("actives : "+actives+" extent : "+extents);
	      ...
	      if (_.include(actives, dimension)) {
	        ...
	        d3.select(element)
	          ...
	          .style('display', function() { 
	            ...
	            //console.log("shd : "+extent[0] <= value && value <= extent[1] ? null : "none");
